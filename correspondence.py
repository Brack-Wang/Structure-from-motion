...
def select_putative_matches(des1, des2):
    ...
    min = np.sort(distance_matrix)[:,0]
    second_min = np.sort(distance_matrix)[:,1]
    min_index = np.argsort(distance_matrix)[:,0]
    matches = [min, min_index, second_min]
    # --------------------------- End your code here   ---------------------------------------------
    ...
    def lowe_ratio_test(matches):
        ...
        min = matches[0]
        min_index = matches[1]
        second_min = matches[2]
        a_constant = 0.4
        lowe_ratio = min - second_min * a_constant
        des1_index = np.argwhere(lowe_ratio < 0)
        des2_index = min_index[des1_index]
        matches = np.hstack((des1_index, des2_index))
        # --------------------------- End your code here   ---------------------------------------------
        return matches
